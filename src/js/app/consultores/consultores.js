...
// };
 ...
 $scope.cargaConsultores = function(){
  ...
  $scope.openEntradaconsultor = function (item) {
    var identificador=item.id;
    MyService.data.identificador = identificador;
      var modalInstance = $modal.open({
        templateUrl: 'modalEntradaconsultor.html',
        controller: 'ModalInstanceCtrl',
        size: 'sm',
        resolve: {
           dato: function () {
    ...
    $scope.calculoExistenciaconsultores(identificador);
  };
  ...
  $scope.openSalida = function (item) {
   ...
   $scope.openSalidaconsultor = function (item) {
    var identificador=item.id;
    MyService.data.identificador = identificador;
      var modalInstance = $modal.open({
        templateUrl: 'modalSalidaconsultor.html',
        controller: 'ModalInstanceCtrl',
        size: 'sm',
        resolve: {
           dato: function () {
...
$scope.openPeso = function (item) {
  ...
  $scope.calculoExistencia=function(idconsultor){
    var entradas=0;
    var salidas=0;
    var existencia=0;
    
    $http.get('http://localhost:1345/entrada/?idconsultor='+idconsultor).then(function (resp) {
      $scope.entradas=resp.data.results;
      for (var i=0;i<$scope.entradas.length;i++){
        entradas=entradas+$scope.entradas[i].cantidad;
        }
        $http.get('http://localhost:1345/salida/?idconsultor='+idconsultor).then(function (resp) {
  ...
  //   var validador=item.identificacion*1000;
  //   $scope.alerts=null;
